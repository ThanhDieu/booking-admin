...
import { getBundleListThunk } from 'store/booking/Bundle';
...
const BundleIBEDisplayList = () => {
  ...
  const { bundlesList, loading } = useAppSelector((state) => state.booking.bundle);
  const PERPAGE = 5;
          ...
          <Badge
            ...
            count={
              stateGetBunleSetting.loading ? (
                ...
                `${bundlesList?.pagination?.total || 0}/${!bundleSettingData
                  ? 0
                  : currentView === ViewMode.Account
                    ? bundleSettingData?.homePageMax
                    : bundleSettingData?.hotelPageMax
            ...
            return (
              <List.Item
                ...
                onClick={() => {
                  ...
                  navigate(
                    `/${property?.extId}/${paths.bundles}/${isTemplate ? paths.bundlesTemplate : paths.bundlesOverview
                    }/${item?.bundleId}`
                  );
                }}
                className={clsx(
