...
import { mediaSlice } from 'store/orion/Media';
...
export default function MediaSelectBox({ form, isReadOnly }: MediaSelectBoxProps) {
  ...
  const { dataFiltered: mediaFilteredData } = useAppSelector((state) => state.orion.media);
  ...
  const modifiedMedia = (data: MediaStrapiType[]): UploadFile<MediaStrapiType>[] => {
    if (data && data.length) {
      return data?.map((media: MediaStrapiType, index: number) => ({
        ...
        status: 'done',
        url: media?.url
          ? `${checkDomainUrl(media.url)}${
              media.formats && Object.keys(media.formats)?.length
