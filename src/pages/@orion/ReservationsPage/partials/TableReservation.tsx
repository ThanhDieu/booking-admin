...
const TableReservation = ({
      ...
      render: (value: string, record: ReservationDetailAppType) => {
        return (
          <Space direction="vertical" size={0}>
            <Typography.Text>
              {`${value || ''} ${record?.data?.primaryGuest?.middleInitial || ''} ${
                record?.data?.primaryGuest?.lastName || ''
              }` || ''}
            </Typography.Text>
            <Typography.Text>{`${
              record?.data?.adults && record?.data?.adults > 1
  ...
  const expandedRowRender = (record: ReservationDetailAppType) => {
    const linkBundleDetail = record?.bundle
      ? `/${record?.property?.extId}/${paths.bundles}/${
          record?.bundle?.isTemplate ? paths.bundlesTemplate : paths.bundlesOverview
        }/${record?.bundle?.id}`
      : '';
