...
const TravelDatePage: React.FC<ReservationDetailPageProps> = ({
  ...
  const { bundleSelected, payloadSearch } = useAppSelector((state) => state.orion.booking);
    ...
    if (bundleSelected) {
      ...
      return [
        ...
        totalPrices.totalUnit && {
          icon: <HomeOutlined />,
          value: `${t('reservation:accommodation')}: ${totalPrices.totalUnit} ${
            bundleSelected?.currency || ''
          }`
        },
        totalPrices.totalService && {
          icon: <CustomerServiceOutlined />,
          value: `${t('reservation:service')}: ${totalPrices.totalService} ${
            bundleSelected?.currency || ''
          }`
        },
        {
